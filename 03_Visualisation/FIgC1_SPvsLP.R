...
# FUSE WA SIM
Eval_FUSE_long_WA_SP = loadRData(file.path(dir_FUSE, "HQLQ_WA_q", "Eval_FUSE.Rdata"))

# FUSE WA SIM
Eval_FUSE_long_WA_LP = loadRData(file.path(dir_FUSE, "HQLQ_WA_q_LP", "Eval_FUSE.Rdata"))


df_sp <- Eval_FUSE_long_WA_SP %>%
  select(Codes, ModelDecisions, KGE_eval = `Eval : KGE :  1`, KGEinv_eval = `Eval : KGE : -1`) %>%
  mutate(Framework = "Sim")

df_lp <- Eval_FUSE_long_WA_LP %>%
  select(Codes, ModelDecisions, KGE_eval = `Eval : KGE :  1`, KGEinv_eval = `Eval : KGE : -1`) %>%
  mutate(Framework = "Obs")

df_all <- bind_rows(df_sp, df_lp)
...
gg1 <- ggplot(df_HQ, aes(x = ModelDecisions, y = Diff)) +
  ...
  labs(
    ...
    title = "(a) Difference in high-flow performance using 50% or 100% of the calibration period as the search period"
  ) +
...
gg2 <- ggplot(df_LQ, aes(x = ModelDecisions, y = Diff)) +
  ...
  labs(
    ...
    title = "(b) Difference in low-flow performance using 50% or 100% of the calibration period as the search period"
  ) +
  coord_cartesian(ylim = c(-0.3, 0.3)) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
ggsave(plot = combined_plot, filename = "99_Figures/FIgC1_SPvsLP.png",
