...
# FUSE WA SIM
Eval_FUSE_long_WA_Q = loadRData(file.path(dir_FUSE, "HQLQ_WA_q", "Eval_FUSE.Rdata"))

# FUSE WA SIM
Eval_FUSE_long_WA_QDQ = loadRData(file.path(dir_FUSE, "HQLQ_WA_qdq", "Eval_FUSE.Rdata"))


df_q <- Eval_FUSE_long_WA_Q %>%
  select(Codes, ModelDecisions, KGE_eval = `Eval : KGE :  1`, KGEinv_eval = `Eval : KGE : -1`) %>%
  mutate(Framework = "Sim")

df_qdq <- Eval_FUSE_long_WA_QDQ %>%
  select(Codes, ModelDecisions, KGE_eval = `Eval : KGE :  1`, KGEinv_eval = `Eval : KGE : -1`) %>%
  mutate(Framework = "Obs")

df_all <- bind_rows(df_q, df_qdq)
...
gg1 <- ggplot(df_HQ, aes(x = ModelDecisions, y = Diff)) +
  ...
  labs(
    ...
    title = "(a) Difference in high-flow performance using only streamflow or both streamflow and its first derivative to construct the feature vector"
  ) +
...
gg2 <- ggplot(df_LQ, aes(x = ModelDecisions, y = Diff)) +
  ...
  labs(
    ...
    title = "(b) Difference in low-flow performance using only streamflow or both streamflow and its first derivative to construct the feature vector"
  ) +
  coord_cartesian(ylim = c(   -0.3, 0.3)) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
ggsave(plot = combined_plot, filename = "99_Figures/FigB1_QvsQDQ.png",
